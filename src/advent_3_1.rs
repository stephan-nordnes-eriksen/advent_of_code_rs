...
pub fn advent() {
    let filename = "./data/advent_3_1.data";
    ...
    let mut map = vec![vec![false; mapSizeUsize * 2]; mapSizeUsize * 2];
    ...
    for tpl_map_two in map2_operations {
        for _i in 0..(tpl_map_two.1) {
            if !(current_x as i32 == mapSize && current_y as i32 == mapSize) {
                temp_distance = (current_x as i32 - mapSize).abs() + (current_y as i32 - mapSize).abs();
                if map[current_x][current_y] && (temp_distance < distance) {
